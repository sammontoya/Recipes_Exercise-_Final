class RecipesController < ApplicationController
  ...
  before_filter :authenticate_user!, except: [:index, :show, :random_recipe]
  ...
  def index
    ...
    # unratedRecipe = Recipe.where(:average_rating => nil)
    # random_recipe = unratedRecipe.sample
  ...
  def add_rating
    ...
    if newRating.valid?
      ...
      @recipe.average_rating = @recipe.ratings.average(:number).round(2)
      @recipe.save
    end
    render 'show.html.erb'
  end
  ...
  def random_recipe
     ...
     respond_to do |format|
       format.json { render json: random_recipe }
     end
   end

   def newest_recipe
     @newest_recipe = Recipe.all.order("created_at asc").last
     @recipes = Recipe.all.order("-average_rating asc")
